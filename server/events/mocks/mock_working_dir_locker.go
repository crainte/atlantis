...
func (mock *MockWorkingDirLocker) TryLock(repoFullName string, pullNum int, repoRelDir string, workspace string) (func(), error) {
	if mock == nil {
		panic("mock must not be nil. Use myMock := NewMockWorkingDirLocker().")
	}
	params := []pegomock.Param{repoFullName, pullNum, repoRelDir, workspace}
...
func (verifier *VerifierMockWorkingDirLocker) TryLock(repoFullName string, pullNum int, repoRelDir string, workspace string) *MockWorkingDirLocker_TryLock_OngoingVerification {
	params := []pegomock.Param{repoFullName, pullNum, repoRelDir, workspace}
...
func (c *MockWorkingDirLocker_TryLock_OngoingVerification) GetCapturedArguments() (string, int, string, string) {
	repoFullName, pullNum, repoRelDir, workspace := c.GetAllCapturedArguments()
	return repoFullName[len(repoFullName)-1], pullNum[len(pullNum)-1], repoRelDir[len(repoRelDir)-1], workspace[len(workspace)-1]
}
