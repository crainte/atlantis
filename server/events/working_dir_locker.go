...
type WorkingDirLocker interface {
	// TryLock tries to acquire a lock for this repo, directory, workspace and pull.
	...
	TryLock(repoFullName string, pullNum int, repoRelDir string, workspace string) (func(), error)
...
func (d *DefaultWorkingDirLocker) TryLock(repoFullName string, pullNum int, repoRelDir string, workspace string) (func(), error) {
	d.mutex.Lock()
	defer d.mutex.Unlock()
	...
	return func() {
		d.unlock(repoFullName, pullNum, repoRelDir, workspace)
	}, nil
}

// Unlock unlocks the workspace for this pull.
func (d *DefaultWorkingDirLocker) unlock(repoFullName string, pullNum int, repoRelDir string, workspace string) {
	d.mutex.Lock()
	defer d.mutex.Unlock()
...
func (d *DefaultWorkingDirLocker) workspaceKey(repo string, pull int, repoRelDir string, workspace string) string {
	return fmt.Sprintf("%s/%s/%s", d.pullKey(repo, pull), repoRelDir, workspace)
}
